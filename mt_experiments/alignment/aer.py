...
class aer():
    ...
    def extract_contribution_matrix(self, hub, contrib_type):
        ...
        for i in range(len(src_bpe_sents)):#len(src_bpe_sents)
            ...
            # Ferrando et al.,  2023
            transformed_vectors_cross = torch.squeeze(hub.get_contributions(src_tensor, tgt_tensor, 't_vectors',
                                                        norm_mode='min_sum')['decoder.encoder_attn'])
            teacher_forced_out = torch.cat([tgt_tensor[1:],torch.tensor([tgt_tensor[0]])]).to(transformed_vectors_cross.device)
            embed_matrix = hub.models[0].decoder.output_projection.weight
            embed_matrix_reduced = torch.index_select(embed_matrix, dim=0, index=teacher_forced_out)
            contributions = torch.einsum('vd,btsd->btvs', embed_matrix_reduced, transformed_vectors_cross).unsqueeze(0)
            
            indices = torch.range(0, teacher_forced_out.size(0)-1,dtype=torch.int64).unsqueeze(-1).repeat(1,contributions.size(-1)).unsqueeze(1).unsqueeze(0).to(teacher_forced_out.device)
            #print('num_layers', self.num_layers)
            indices = indices.repeat(self.num_layers,1,1,1)
            #print('indices', indices.shape)
            cross_attn_contrib_proj = torch.gather(contributions.squeeze(),2,indices).squeeze().detach()[:,:,:-1]
            encoder_alti = alti_model['encoder.self_attn'][-1].detach() # last layer encoder alti
            cross_attn_contrib_proj_alti = torch.matmul(cross_attn_contrib_proj,encoder_alti.unsqueeze(0))
            #print('cross_attn_contrib_proj_alti', cross_attn_contrib_proj_alti.shape)
            ...
            #print('combined_alti_enc_cross', combined_alti_enc_cross.shape)
            ...
            alti_model_attn_w = torch.squeeze(hub.get_contributions(src_tensor, tgt_tensor, contrib_type='attn_w',
                                                        norm_mode='sum_one')['decoder.encoder_attn']).detach()
            # Vector Norms (Kobayashi., 2021)
            vector_norms_cross = torch.squeeze(hub.get_contributions(src_tensor, tgt_tensor, 'l2',
            ...
            extract_matrix.append({"alti":total_alti_pred_src, "decoder.encoder_attn": cross_attn_contributions,
                                    ...
                                    "cross_attn_contributions_proj": cross_attn_contrib_proj,
                                    "cross_attn_contrib_proj_alti": cross_attn_contrib_proj_alti,
                                    "vector_norms_cross": vector_norms_cross})
    ...
    def extract_alignments(self, final_punc_mark=False, ignore_eos=False):
