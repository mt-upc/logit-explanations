...
class FairseqTransformerHub(GeneratorHubInterface):
    ...
    def __get_contributions_module(self, layer_inputs, layer_outputs, contrib_type, module_name):
        ...
        if pre_layer_norm:
            ...
            transformed_vectors = l_transform(out_qv_pre_ln, w_ln)*ln_std_coef # (batch,tgt_len,src_len,embed_dim)
            dense_bias_term = l_transform(b_o, w_ln)*ln_std_coef # (batch,src_len,1,embed_dim)
            attn_output = transformed_vectors.sum(dim=2) # (batch,tgt_len,embed_dim)
            resultant = attn_output + dense_bias_term.squeeze(2) + b_ln # (batch,tgt_len,embed_dim)
        ...
        elif contrib_type == 't_vectors':
            return transformed_vectors.unsqueeze(1), None
    ...
    def get_contribution_rollout(self, src_tensor, tgt_tensor, contrib_type='l1', norm_mode='min_sum', **contrib_kwargs):
        ...
        def rollout(C, C_enc_out):
            ...
            for i in range(1, len(C)):
                ...
                ) + C_roll[i, :, :src_len]    # Cross_attn*encoder rollout
